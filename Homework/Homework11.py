...
arr = sorted(list(filter(lambda x: len(x)==4 or x[0]=='S', days)))
print('\n'.join(arr))
...
for i in sorted(models, key=lambda dictt: dictt['color']):
...
while True:
    ...
    if s == 'конец':
        break
    dictt[int(s.split(': ')[1])] = s.split(': ')[0]
for i in sorted(dictt.keys(), reverse=True):
    print(dictt[i])
...
for i in dictt.keys():
    ...
    if dictt[i]<minn:
        minn=dictt[i]
        name_min=i
print(f"{name_max}, {maxx}")
print(f"{name_min}, {minn}")
...
for i in range(n):
    s=input()
    if not s.split()[1] in dictt:
        dictt[s.split()[1]] = [s.split()[0]]
    else:
        dictt[s.split()[1]].append(s.split()[0])
m=int(input())
arr=[]
for i in range(m):
    arr.append(input())
...
for i in range(n):
    s=input()
    data.append(dict(name=s.split()[0], day=s.split()[1], month=s.split()[2]))
m=int(input())
arr=[]
for i in range(m):
    arr.append(input())
